...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for i in range(len(parallel_corpus)):
            lis.append(np.divide(self.translation_probs[np.ix_(parallel_corpus[i].source_tokens, parallel_corpus[i].target_tokens)],np.sum(self.translation_probs[np.ix_(parallel_corpus[i].source_tokens, parallel_corpus[i].target_tokens)],axis=0)))
        return lis
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for i in range(len(posteriors)):
            L += np.sum(posteriors[i] * np.log(len(parallel_corpus[i].source_tokens)**(-1)*self.translation_probs[np.ix_(parallel_corpus[i].source_tokens,parallel_corpus[i].target_tokens)]+10**(-23)) -posteriors[i] *np.log(posteriors[i]+10**(-23)))
        return L
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for i in range(len(parallel_corpus)):
            c_s =np.unique(parallel_corpus[i].source_tokens,return_index=True,return_counts=True)
            c_t = np.unique(parallel_corpus[i].target_tokens,return_index=True,return_counts=True)
            self.translation_probs[np.ix_(c_s[0],c_t[0])] +=\
            np.multiply(posteriors[i][np.ix_(c_s[1],c_t[1])],c_s[2].reshape(-1,1)*c_t[2])
    ...
    def align(self, sentences):
        ...
        for i in range(len(sentences)):
            al = list(map(tuple,np.vstack((np.argmax(self.translation_probs[np.ix_(sentences[i].source_tokens, sentences[i].target_tokens)]/np.sum(self.translation_probs[np.ix_(sentences[i].source_tokens, sentences[i].target_tokens)], axis=0), axis = 0) + 1, np.arange(1, sentences[i].target_tokens.shape[0] + 1))).T.tolist()))
            lm.append(al)
        return lm
...
class WordPositionAligner(WordAligner):
    ...
    def _get_probs_for_lengths(self, src_length: int, tgt_length: int):
        ...
        """
        if (src_length, tgt_length) not in list(self.alignment_probs.keys()):
    ...
    def _e_step(self, parallel_corpus):
        e = []
        for i in range(len(parallel_corpus)):
            haha = np.multiply(self._get_probs_for_lengths(len(parallel_corpus[i].source_tokens),len(parallel_corpus[i].target_tokens)),self.translation_probs[np.ix_(parallel_corpus[i].source_tokens,parallel_corpus[i].target_tokens)])
            e.append(haha/np.sum(haha,axis=0))
        return e
    def _compute_elbo(self, parallel_corpus, posteriors):
        L = 0
        for i in range(len(parallel_corpus)):
            L+=np.sum(posteriors[i]*\
                 (np.log(self.alignment_probs[len(parallel_corpus[i].source_tokens),len(parallel_corpus[i].target_tokens)]*self.translation_probs[np.ix_(parallel_corpus[i].source_tokens,parallel_corpus[i].target_tokens)]+10**(-23)) -\
                  np.log(posteriors[i]+10**(-23))))
        return L
    def _m_step(self, parallel_corpus, posteriors):
        ...
        for i in range(len(parallel_corpus)):
            c_s =np.unique(parallel_corpus[i].source_tokens,return_index=True,return_counts=True)
            c_t = np.unique(parallel_corpus[i].target_tokens,return_index=True,return_counts=True)
            self.translation_probs[np.ix_(c_s[0],c_t[0])] += np.multiply(posteriors[i][np.ix_(c_s[1],c_t[1])],c_s[2].reshape(-1,1)*c_t[2]) 
        self.translation_probs /= np.sum(self.translation_probs ,axis=1).reshape(-1,1)
        post_shape = np.array(list(map(lambda x: x.shape,posteriors)))
        for i,j in list(self.alignment_probs.keys()):
            self.alignment_probs[(i,j)]= np.array(posteriors)[np.where((post_shape[:,0] == i) & (post_shape[:,1]==j))].sum(keepdims=True)[0]
            self.alignment_probs[(i,j)]/= np.sum(self.alignment_probs[(i,j)], axis=0)
        return self._compute_elbo(parallel_corpus, posteriors)